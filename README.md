...
* **因数轨道**：底部有 1~9 的因数。
* **占领格子**：玩家轮流移动其中一个因数，使新的乘积对应棋盘上的格子，从而占领该格子。

## 详细规则

1. **开局第 1 步**：玩家 1 选择一个因数，不占领格子。
2. **第 2 步**：玩家 2 选择一个因数，产生第一个乘积并占领对应格子。
3. **常规回合**：玩家轮流移动一个因数以占领新的格子。新乘积对应的格子必须未被占领。
4. **获胜条件**：率先在棋盘上形成连续的连线（横/纵/斜）， 可在设置中自定义几连胜。如果当前玩家移动因数到任何位置产生的乘积都已被占领，则该玩家无路可走，被迫投降
